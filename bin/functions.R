...
#Marimekko plot without stats
            mosaicGG2 <- function(data, X, FILL, colors = colors_many, rect_col = 'white', line_width = 0.25, text_size = 5) {
              require(dplyr)
              require(reshape2)
              #require(ggthemes)
              # Proportions in raw data
              ...
              g <- ggplot(DF_all, aes(ymin = ymin,  ymax = ymax, xmin = xmin,
                ...
                geom_rect(col = rect_col, lwd = line_width) +
                geom_text(aes(x = xposn, label = X),
                          y = 1, size = text_size, angle = 90, hjust = 1, show.legend = FALSE,check_overlap = T) +
                geom_text(aes(x = max(xmax),  y = yposn, label = FILL),
                          size = text_size, hjust = 1, show.legend = FALSE,check_overlap = T) +
